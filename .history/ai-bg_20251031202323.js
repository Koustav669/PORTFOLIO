...
(function(){
  ...
  const usable = Math.max(32, h - margin*2);
    ...
    const baseY = margin + usable * 0.15;
    for(let k=0;k<keyCount;k++){
      ...
      const height = (0.2 + 0.8 * (0.6*h1 + 0.4*h2 + depth*0.2)) * (usable * 0.6);
      ...
      roundRect(ctx, kx, baseY + (usable*0.5 - height), kw, height, 6);
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.moveTo(kx + 2, baseY + (usable*0.5 - height) + 1);
      ctx.lineTo(kx + kw - 2, baseY + (usable*0.5 - height) + 1);
      ctx.stroke();

      // subtle glow under the bar
      ctx.fillStyle = `rgba(${col[0]},${col[1]},${col[2]},0.035)`;
      ctx.fillRect(kx - 6, baseY + (usable*0.5 - height) + height + 6, kw + 12, 10);
    }
