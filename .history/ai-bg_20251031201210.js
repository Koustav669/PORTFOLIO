...
(function(){
  ...
  function updateOverlayOpacity(){
    try{
      ...
      // reduce overlay fully in debug so canvas is clearly visible
      ov.style.opacity = debug ? '0.08' : '1';
      // also clear overlay background when debugging to avoid any blockage
      if(debug) ov.style.background = 'transparent'; else ov.style.background = '';
      console.log('[ai-bg] debug overlay state ->', debug);
    }catch(e){}
  }
  ...
  ctx.strokeStyle = debug ? `rgba(60,180,255,0.6)` : `rgba(60,180,255,0.22)`;
    ...
    // Debug visual marker (very obvious) so the user can see the canvas is rendering
    if(debug){
      ctx.save();
      ctx.fillStyle = 'rgba(0,200,255,0.06)';
      ctx.fillRect(cx - baseSize*0.9, cy - baseSize*0.5, baseSize*1.8, baseSize*1.0);
      ctx.fillStyle = 'rgba(0,240,255,0.12)';
      ctx.font = '600 14px Poppins, sans-serif';
      ctx.fillText('DEBUG: TESSERACT VISIBLE', cx - baseSize*0.82, cy - baseSize*0.22);
      ctx.restore();
    }
